...
bool events::out::state(gameupdatepacket_t* packet) {
    if (!server->m_world.connected)
        return false;

    server->m_world.local.pos = vector2_t{ packet->m_vec_x, packet->m_vec_y };
    ...
    switch (hs::hash32(func.c_str())) {
        ...
        case fnv32("OnRemove"): {
            ...
            if (text.find("netID|") == 0) {
                ...
                if (netid == server->m_world.local.netid)
                    server->m_world.local = {};

                auto& players = server->m_world.players;
        ...
        case fnv32("OnSpawn"): {
            ...
            if (name && netid && onlineid) {
                ...
                if (meme.find("type|local") != -1) {
                    ...
                    server->m_world.local = ply;
                }
                server->m_world.players.push_back(ply);
...
bool events::in::sendmapdata(gameupdatepacket_t* packet) {
    ...
    server->m_world.name = std::string(name);
    server->m_world.connected = true;
    delete[] name;
    PRINTC("world name is %s\n", server->m_world.name.c_str());
    return false;
}
